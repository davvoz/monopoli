...
it('roundTest should returns true if all rounds dont contain an error', () => { 
  ...
  let players = TestMonopoli.randomlyStart([mario,anna,carlo]); //ordina i giocatori casualmente
  ...
  for( let i = 0 ; i < 20 ; i++){ // simula 20 turni
...
it('RandomlyStart should returns true if the array^s length is 5', () => { 
  ...
  const poolPLayers = [{name : "Mario" , position : 0,index:1},{name : "Aldo" , position : 0,index:0},{name : "Anna" , position : 0,index:2},{name : "Clara" , position : 0,index:3},{name : "Ugo" , position : 0,index:4}];
...
it('isGoTest should returns true if a player pass or lands from "GO" cell', () => { 
  console.log('second relase')
    const mario = { name : "Mario" , index : 0 , position : 35};
